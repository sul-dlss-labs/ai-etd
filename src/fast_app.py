...
# spacy_ner.add_pipe(
# Entity(keywords_dict=generate_labels(fast_geo), label="FAST_GEOGRAPHIC"))
...
if st.button("Random Thesis"):
    ...
    for text in entities:
        st.sidebar.markdown(
            f"{text} {topic_entity.keyword_processor.get_keyword(text)}"
        )
...
#    st.sidebar.multiselect(options=chron_output,
#                          label='Select FAST Chronology')
